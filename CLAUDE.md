...
### 3-7. 실서비스 목표 프로젝트 운영 원칙 (2026-05-25 추가)

`experiments/<name>/` 학습 프로젝트는 **두 가지 동기 중 하나로 운영**한다:

**(a) 학습 완전성 동기** — 책의 모든 패턴·알고리즘을 시연하기 위해 의도적으로 추가 구현. knot cycle 0~7의 원래 방향. cycle별로 sub-spec·plan·wiki section 누적.

**(b) 실서비스 동기** — 실제 운영 가능한 서비스라면 어떻게 설계할지 기준. 책 패턴 중 **실제 필요한 것만** 적용, 학습용으로만 의미 있는 것은 제외.

knot은 2026-05-25에 cycle 7 종료 후 (a) → (b)로 방향 전환. 앞으로의 `experiments/` 신규 프로젝트는 기본 (b) 동기로 시작.

**실서비스 동기 프로젝트의 원칙**

1. **책의 패턴 중 실제 필요한 것만** — "다 시연하려고 추가"는 금지. 추가 시 명시적 이유(엔드포인트 특성, 비즈니스 요구) 필요
2. **학습 동기로 추가한 코드 발견 시 즉시 제거** — 회고에서 정직성 점검으로 잡힐 수 있음
3. **적용된 개념·설계 선택 이유는 반드시 기록** — spec과 wiki/projects 페이지에 "왜 이 선택인가", "다른 옵션은 왜 안 골랐나"를 명시
4. **실세계 사례 인용** — Stripe/AWS/Cloudflare/nginx 등 실서비스가 같은 결정을 어떻게 하는지 표나 인용으로 비교
5. **방향 전환(pivot) 시 기존 작업 보존** — revisionist 안 됨. 학습 단계 산출물은 archive 형태로 두고, 현재 상태 별도 표시 (knot이 예: cycle 0~7 + "Pivot" 섹션)
6. **제거 결정도 학습 자산** — 무엇을 왜 뺐는지 기록하면 다음 프로젝트의 재발 방지

**문서 분담** (실서비스 동기 프로젝트)

| 어디 | 무엇 |
|---|---|
| `experiments/<name>/DESIGN.md` 또는 README | 현재 상태의 실서비스 설계 narrative (책 4단계 프레임워크 등) |
| `wiki/projects/<name>.md` | 시간순 진행 일지 + 결정 이력 + pivot 기록 |
| `docs/specs/...-design.md` | 시스템 설계 결정 (변경 시 새 sub-spec 추가) |
| `docs/plans/...` | TDD 구현 계획 |
