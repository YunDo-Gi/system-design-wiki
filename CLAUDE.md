...
### 3-6. 실서비스 목표 프로젝트 운영 원칙

`experiments/<name>/` 디렉터리에 두는 코드 프로젝트는 **실제 운영 가능한 서비스 기준**으로 설계·진화한다. knot이 첫(현재 유일) 사례.

**원칙**:

1. **책의 패턴 중 실제 필요한 것만 적용** — "다 시연하려고 추가" 금지. 추가 시 명시적 동기 (엔드포인트 특성·비즈니스 요구) 필요
2. **학습 동기로만 추가된 코드 발견 시 즉시 제거** — 정직성 우선
3. **적용된 개념·설계 선택 이유는 반드시 기록** — **챕터별로 구분해서** DESIGN.md에. "왜 이 선택", "다른 옵션은 왜 안 골랐나"
4. **실세계 사례 인용** — Stripe/AWS/Cloudflare/nginx 등 실서비스가 같은 결정을 어떻게 하는지 표·인용으로 비교

**문서 분담**:

| 어디 | 무엇 |
|---|---|
| `experiments/<name>/DESIGN.md` | **챕터별 섹션 구성** — 각 챕터에서 적용한 개념 + 설계 선택 이유 + 실세계 비교. **현재 상태만** (과거 변경·제거 이력 기록 안 함, 회고 비정책) |
| `experiments/<name>/README.md` | 빠른 시작·디렉터리·실행법 |
| `docs/specs/...-design.md` | 시스템 설계 결정. 새 기능 추가 시 새 sub-spec 파일 |
| `docs/plans/...` | TDD 구현 계획. 새 기능 단위로 작성 |

**기능 추가 시 흐름**: 새 sub-spec + plan 작성 → 구현 → DESIGN.md 해당 챕터 섹션 갱신. 사이클 번호·회고 섹션·진행 일지 없음. "기능 X 추가"가 단위.

**`wiki/projects/`는 폐지** — knot이 학습 단계에 사용했던 위키 카테고리. 회고·진행 일지를 안 쓰는 정책으로 전환되어 카테고리 자체 불필요. CLAUDE.md §2 디렉터리 규약에서도 제거.
