...
`# 제목 (영문)` → `## 핵심 takeaway` (3-5 bullet) → **개념 기반 섹션 본문** (예: `## 개요`, `## 위치 / 배치`, `## 핵심 알고리즘`, `## 분산 환경의 난제`, `## 운영` 등 챕터 주제에 맞게 자유 구성. **책의 "Step 1/2/3/4" 분절은 답습하지 않는다** — 그건 책이 면접 가이드라 그렇게 구조화된 것이고 본 위키는 학습 관점이다) → `## 등장 개념` (`[[링크]]` 목록) → `## 등장 기술` (`[[링크]]` 목록) → (선택) `## 면접 관점 메모` (3줄 이내).
