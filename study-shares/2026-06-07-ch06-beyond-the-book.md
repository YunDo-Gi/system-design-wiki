...
모든 쓰기에 timestamp를 붙이고, 충돌이 나면 timestamp가 가장 큰 값을 택한다. 버전 족보를 추적하지 않고 숫자 비교 한 번으로 끝나니 단순하고 빠르다.
...
대신 동시에 일어난 두 쓰기 중 진 쪽은 흔적 없이 사라진다. 알림도 머지 기회도 없어서 aphyr는 이를 "정보를 비결정적으로 파괴한다"고 표현한다. 게다가 어느 쪽이 나중인지를 timestamp로 정하기 때문에, 노드 간 시계가 어긋나면(clock skew) 실제로 더 늦게 일어난 쓰기가 지는 일도 생긴다. 그래서 NTP 동기화에 의존한다.

프로필 필드나 캐시, 센서 최신값처럼 둘 중 하나를 잃어도 무방한 데이터엔 충분하지만, 장바구니처럼 양쪽 쓰기가 각자 의미를 담는 데이터엔 맞지 않는다.

#### CRDT (Conflict-free Replicated Data Type)

충돌을 사후에 푸는 대신, 자료구조의 머지 연산을 교환법칙·결합법칙·멱등성을 만족하도록 설계한다. 그러면 복제본들이 변경을 어떤 순서로, 몇 번 중복해서 받아도 결국 같은 값으로 수렴한다(수학적으로 join semilattice). 클라이언트가 머지 로직을 짤 필요가 없다는 게 핵심이다.

대신 아무 데이터에나 되는 건 아니고, 머지가 잘 정의되는 타입들이 따로 준비돼 있다. 증가 전용 카운터 G-Counter(머지는 서버별 카운트의 최댓값), 증가·감소가 되는 PN-Counter, 추가/삭제가 되는 집합 OR-Set(항목마다 고유 태그를 붙여 동시 추가·삭제 시 추가를 살림), 그리고 LWW-Register — 사실 앞의 LWW도 머지가 정의된 단순한 CRDT의 일종이다.

OR-Set으로 앞의 apple/banana를 보면, A가 `apple#t1`, B가 `banana#t2`를 태그와 함께 추가했으니 머지는 태그 단위 합집합이 되어 `{apple, banana}`로 합쳐진다. 한쪽도 잃지 않는다.
...
다만 머지가 정의된 타입에 한정되고(임의 비즈니스 로직은 안 된다), 태그나 tombstone 같은 메타데이터가 쌓인다. 협업 편집(Figma·Notion), Redis CRDT, Automerge가 이 방식을 쓴다.
