...
**시각 동기화**: sliding window 계열은 timestamp 기반이라 서버 시계가 어긋나면 윈도우 계산이 깨진다. 표준 해법은 Lua 안에서 `redis.call('TIME')` 사용 (모든 클라이언트가 단일 Redis 시각을 봄). 더 깊은 인프라 측면은 [[network-time-protocol]] 참조 — ch07 [[snowflake-id]]도 같은 NTP 가정에 의존한다.
